...
const handleMouseMove = (e) => {
    ...
    // 2. 3D Tilt Effect - REDUCED RANGE
    const { left, top, width, height } = cardRef.value.getBoundingClientRect();
    const centerX = left + width / 2;
    const centerY = top + height / 2;
    
    // Reduced multiplier from 10 to 4 for subtle movement
...
// Add global event listener for background parallax if desired, 
// for now keeping it localized to the card area for performance.
const containerRef = ref(null);
const bgParallax = ref({ x: 0, y: 0 });

const updateParallax = (e) => {
    // Reduced parallax movement as well
...
<template>
    ...
    <div 
        ref="containerRef"
        class="min-h-screen grid items-center justify-center relative overflow-hidden bg-gray-100 dark:bg-[#030303] transition-colors duration-500"
    ...
    >
        ...
        <div class="absolute inset-0 z-0 overflow-hidden pointer-events-none transition-transform duration-200 ease-out" 
        ...
        <div class="absolute top-6 right-6 z-50 p-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20 shadow-lg hover:bg-white/20 transition-all duration-300">
        ...
        <div 
            ref="cardRef"
            class="relative z-40 w-[90vw] max-w-md transform-gpu transition-transform duration-100 ease-out"
