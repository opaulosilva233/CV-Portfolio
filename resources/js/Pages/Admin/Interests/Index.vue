...
<template>
    <Head :title="__('Interests Management')" />
        ...
        <template #header>
            <div class="flex items-center justify-between w-full gap-3">
                ...
                <Link :href="route('admin.interests.create')" class="flex-shrink-0 px-3 py-2 sm:px-5 sm:py-2.5 bg-cyan-600/80 hover:bg-cyan-500 border border-cyan-500/50 rounded-xl font-semibold text-xs text-white uppercase tracking-wider shadow-[0_0_15px_rgba(6,182,212,0.4)] transition-all duration-300">
                    <span class="hidden sm:inline">{{ __('Add New Interest') }}</span>
                ...
                <div class="bg-white/5 backdrop-blur-md border border-white/10 shadow-xl sm:rounded-2xl overflow-hidden">
                    <div class="p-6">
                        <div v-if="interests.length === 0" class="text-center py-12">
                            <p class="text-gray-400 italic">{{ __('No interests found. Click the button above to add one.') }}</p>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-white/10">
                                <thead class="bg-white/5">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">{{ __('Interest') }}</th>
                                ...
                                <tbody class="divide-y divide-white/10 bg-transparent">
                                    ...
                                    >
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span v-if="interest.is_active" class="px-2 py-1 bg-green-500/10 text-green-400 border border-green-500/30 rounded text-xs">{{ __('Active') }}</span>
                                            <span v-else class="px-2 py-1 bg-red-500/10 text-red-400 border border-red-500/30 rounded text-xs">{{ __('Inactive') }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <Link :href="route('admin.interests.edit', interest.id)" class="text-cyan-400 hover:text-cyan-300 mr-4 transition-colors">{{ __('Edit') }}</Link>
                                            <Link :href="route('admin.interests.destroy', interest.id)" method="delete" as="button" class="text-red-400 hover:text-red-300 transition-colors" :onsubmit="`return confirm('${__('Are you sure?')}')`">{{ __('Delete') }}</Link>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
