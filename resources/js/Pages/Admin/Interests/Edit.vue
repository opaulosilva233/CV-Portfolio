        ...
        <template #header>
            <div class="flex items-center justify-between w-full gap-3">
                <h2 class="text-base sm:text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-cyan-400 truncate flex-1">
                    {{ __('Edit Interest') }}: {{ interest.name }}
                </h2>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="pt-6 border-t border-white/10 flex gap-4">
                            <button type="submit" :disabled="form.processing" 
                                ...
                                {{ form.processing ? __('Updating...') : __('Update Interest') }}
                            </button>
                            <Link :href="route('admin.interests.destroy', interest.id)" method="delete" as="button" 
                                class="px-8 py-3 bg-red-600/20 hover:bg-red-600/40 border border-red-500/30 text-red-400 font-bold rounded-xl transition-all duration-300" :onsubmit="`return confirm('${__('Are you sure?')}')`">
                                {{ __('Delete') }}
                            </Link>
                        </div>
                    </form>
