...
const form = useForm({
    ...
    in_progress: props.project.in_progress,
...
onMounted(() => {
    // Fill gallery with existing images
    if (props.project.gallery) {
        props.project.gallery.forEach(img => {
            galleryItems.value.push({
                id: 'exp_' + nextItemId.value++,
...
const addGalleryItem = (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const id = 'new_' + nextItemId.value++;
        ...
        reader.onload = (ev) => {
            galleryItems.value.push({
                ...
                description: '',
                is_principal: galleryItems.value.length === 0,
                is_existing: false,
            });
        };
...
const removeGalleryItem = (item) => {
    if (item.is_existing) {
        form.remove_images.push(item.filename);
    }
    galleryItems.value = galleryItems.value.filter(i => i.id !== item.id);
    
    if (galleryItems.value.length > 0 && !galleryItems.value.some(i => i.is_principal)) {
...
const submit = () => {
    ...
    const metadata = {
        descriptions: {},
        // We'll also tell the controller which new item or existing item is principal
    };
    ...
    galleryItems.value.forEach((item) => {
        if (!item.is_existing) {
            ...
            const key = 'new_' + newFileIndex;
            metadata['new_' + newFileIndex] = { is_principal: item.is_principal };
            metadata.descriptions[key] = item.description;
            newFileIndex++;
        } else {
            // Existing file
            const nameWithoutExt = item.filename.split('.').slice(0, -1).join('.');
            metadata.descriptions[nameWithoutExt] = item.description;
            if (item.is_principal) {
                // If an existing file is now principal, we need to handle that.
                // The backend logic handles 'principal' file specially.
                // If a new file is principal, it replaces the old one.
                // If an existing file (that wasn't principal) is now principal, 
                // we'd need a rename on backend. 
                // Let's simplify: if an existing item is selected as principal, 
                // we'll send its name.
                metadata.is_principal_existing = nameWithoutExt;
            }
        }
    });
                    ...
                    <form @submit.prevent="submit" class="space-y-8">
                        ...
                        <div>
                            ...
                            <div v-if="galleryItems.length === 0" class="flex flex-col items-center justify-center py-12 px-6 rounded-2xl bg-gray-900/50 border-2 border-dashed border-white/10">
                                ...
                                <p class="text-gray-500 text-sm">No images. Add prints of your work!</p>
                            </div>
                                ...
                                <div v-for="item in galleryItems" :key="item.id" class="relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden group">
                                    <div class="aspect-video relative overflow-hidden bg-black/40">
                                        ...
                                        <div class="absolute top-2 right-2 flex gap-2">
                                            <button type="button" @click="removeGalleryItem(item)" class="p-1.5 bg-red-500/80 hover:bg-red-500 rounded-lg text-white shadow-lg transition-colors">
                                        ...
                                        <div v-if="!item.is_existing" class="absolute top-2 left-2 px-1.5 py-0.5 bg-green-500 text-[8px] uppercase font-black rounded text-white shadow-lg">New</div>
                                    </div>
                                    <div class="p-3">
                                        <textarea 
